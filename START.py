...
INPUT_DIR = ".wav sounds you want dubbed directory"
...
def main():
    ...
    for file_name in os.listdir(input_folder):
        ...
        text = translation_map.get(name_without_ext)

        # If no translation was found, check the reference CSV
        if not text and name_without_ext in reference_map:
            full_subtitle = reference_map[name_without_ext]
            text = full_subtitle.split("\t")[0]  # Take only the first half

            print(f"✅ Found fallback translation from REFERENCE.CSV: {text}")
